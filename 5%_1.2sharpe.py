...
def run_backtest(csv_path, initial_funds=100000.0):
    ...
    # 5 Days in Hours (assuming 1h candles)
    LOOKBACK_WINDOW = 150 
    ...
    for i in range(len(df) - 1):
        ...
        if pd.isna(current_row['volume_z']) or pd.isna(current_row['atr']):
            portfolio_values.append(engine.balance)
            continue

        # --- UPDATED STRATEGY LOGIC ---
        # Net price change over the last 120 hours
        price_change_5d = current_row['close'] - df.iloc[i - LOOKBACK_WINDOW]['close']

        if price_change_5d < 0:
            target_signal = -1 # Short if net change is negative
        else:
            target_signal = 1  # Long if net change is positive or zero

        # Reset counters as strategy logic changed
        volume_trade = False
        trend_trade = False
        ...
        # Increment report counters based on direction for tracking
        if target_signal == 1:
            trend_trades += 1
        else:
            sideways_trades += 1
